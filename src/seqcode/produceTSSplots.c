/*************************************************************************
*                                                                        *
*   Module: produceTSSplots                                              *
*                                                                        *
*   ProduceTSSplots main program                                         *
*                                                                        *
...
int main (int argc, char *argv[])
{
  ...
  readargv_produceTSSplots(argc,argv,ChromInfoFile,
			   RefgeneFile,ReadsFile,
			   GenesFile,TrackName,
			   &L);
  printSeqCodeHeader(PRODUCETSSPLOTS);
  StartingTime(mess,m);
  printMess(mess);
  ...
  sprintf(OutputDirName,"%s_TSSplot_%d",TrackName,L);
  ...
  /* Output files */
  sprintf(OutputFileName,"%s/TSSprofile_%s_%d.txt",OutputDirName,TrackName,L);
  ...
  sprintf(PlotFileName,"%s/PlotTSSprofile_%s_%d.pdf",OutputDirName,TrackName,L);
  ...
  m->nGenes = ProcessTargetGenes(GenesFile,Transcripts,
				 ChrNames,ChrSizes,
				 GeneNames,GeneChrs,
				 L,READS,REGION,
				 OutputFileName,
  ...
  if (m->nGenes>0)
    {
      ...
      sprintf(RscriptFileName,"%s/RscriptTSSprofile_%s_%d.txt",OutputDirName,TrackName,L);
      sprintf(mess,"Preparing the Rscript file %s",RscriptFileName);
      printRes(mess);
      
      CreateRscript(OutputFileName,
		    RscriptFileName,
		    PlotFileName,
		    TrackName,L,
		    m->nReads,
