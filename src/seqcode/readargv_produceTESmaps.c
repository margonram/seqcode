/*************************************************************************
*                                                                        *
*   Module: readargv:produceTESmaps                                      *
*                                                                        *
*   Read set up options and filenames from user input (produceTESmaps)   *
*                                                                        *
			     ...
			     int* L)
{
  ...
  sprintf(USAGE[0],
	  ...
	  PRODUCETESMAPS);
  sprintf(USAGE[1],
          "NAME\n\t%s - a program to produce ChIPseq/ATACseq/RNAseq heat maps around the TES.\n",
          PRODUCETESMAPS);
  sprintf(USAGE[2],
          "SYNOPSIS\n\t%s [-bBfFH <Rcolor>][-i <real>][-l <bp>][-n][-w <bp>][-v][-h]\n\t<chrom_info> <refGene.txt> <SAM file> <targetGenes.txt> <name> <length>\n",
          PRODUCETESMAPS);
  sprintf(USAGE[3],
  ...
  while ((c = getopt(argc,argv,"b:B:f:F:H:il:ns:t:w:vh")) != -1)
    switch(c)
      {
      case 'b':
