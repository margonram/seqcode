...
int main (int argc, char *argv[])
{
  ...
  sprintf(OutputDirName,"%s_PEAKplot_%d",TrackName,L);
  ...
  /* PEAK plots */
  sprintf(OutputFileName,"%s/PEAKprofile_%s_%d.txt",OutputDirName,TrackName,L);
  sprintf(mess,"Filename for the PEAK average profile %s",OutputFileName);
  printRes(mess);

  sprintf(PlotFileName,"%s/PlotPEAKprofile_%s_%d.pdf",OutputDirName,TrackName,L);
  ...
  if (m->nPeaks>0)
    {
      ...
      sprintf(RscriptFileName,"%s/RscriptPEAKprofile_%s_%d.txt",OutputDirName,TrackName,L);
