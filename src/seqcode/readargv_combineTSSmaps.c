			     ...
			     int* L)
{
  ...
  while ((c = getopt(argc,argv,"b:B:f:F:g:H:il:n:w:vh")) != -1)
    switch(c)
      {
      case 'b':
  ...
  strcpy(ProfileFile1,argv[optind+2]);
  strcpy(ProfileFile2,argv[optind+3]);
  strcpy(GenesFile,argv[optind+4]);
  strcpy(TrackName1,argv[optind+5]);
  strcpy(TrackName2,argv[optind+6]);
