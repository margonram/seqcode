...
{
  char PeaksFileA[FILENAMELENGTH], PeaksFileB[FILENAMELENGTH];
  char NameA[MAXSTRING],NameB[MAXSTRING];
  ...
  readargv_matchpeaks(argc,argv,PeaksFileA,PeaksFileB,NameA,NameB);
  ...
  printMess("2. Creating output files");

  sprintf(OutputDirName,"%s_%s_matchpeaks",NameA,NameB);
