/*************************************************************************
*                                                                        *
*   Module: readargv_produceTESplots                                     *
*                                                                        *
*   Read set up options and filenames from user input (produceTESplots)  *
*                                                                        *
...
void printHelp_produceTESplots()
{
  fprintf(stderr,"%s\n",USAGE[0]);
  fprintf(stderr,"%s\n",USAGE[1]);
  fprintf(stderr,"%s\n",USAGE[2]);
			      ...
			      int* L)
{
  ...
  sprintf(USAGE[0],
	  ...
	  PRODUCETESPLOTS);
  sprintf(USAGE[1],
	  "NAME\n\t%s - a program to produce averaged ChIPseq/ATACseq/RNAseq metaplots around the TES.\n",
          PRODUCETESPLOTS);
  sprintf(USAGE[2],
	  "SYNOPSIS\n\t%s [-bBfF <Rcolor>][-l <bp>][-s <reads>][-u][-w <bp>][-v][-h]\n\t<chrom_info> <refGene.txt> <SAM file> <targetGenes.txt> <name> <length>\n",
	  PRODUCETESPLOTS);
  sprintf(USAGE[3],
  ...
  while ((c = getopt(argc,argv,"b:B:f:F:l:s:uw:vh")) != -1)
    switch(c)
      {
      case 'b':
	...
	break;
      ...
      case 'v': VRB++;
	break;
      case 'h': printHelp_produceTESplots();
	exit(1);
	break;
      }
