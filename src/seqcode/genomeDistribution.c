...
int main (int argc, char *argv[])
{
  ...
  sprintf(OutputDirName,"%s_GENOMEchart",TrackName);
  ...
  /* Output files */
  sprintf(OutputFileName,"%s/GENOMEchart_%s.txt",OutputDirName,TrackName);
  sprintf(mess,"Filename for the marked peak file %s",OutputFileName);
  printRes(mess);

  sprintf(RscriptFileName,"%s/RscriptGENOMEchart_%s.txt",OutputDirName,TrackName);
  sprintf(mess,"Preparing the Rscript file %s",RscriptFileName);
  printRes(mess);

  sprintf(PlotFileName,"%s/PlotGENOMEchart_%s.pdf",OutputDirName,TrackName);
