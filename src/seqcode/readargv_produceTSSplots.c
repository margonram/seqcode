/*************************************************************************
*                                                                        *
*   Module: readargv_produceTSSplots                                     *
*                                                                        *
*   Read set up options and filenames from user input (produceTSSplots)  *
*                                                                        *
...
void printHelp_produceTSSplots()
{
  fprintf(stderr,"%s\n",USAGE[0]);
  fprintf(stderr,"%s\n",USAGE[1]);
  fprintf(stderr,"%s\n",USAGE[2]);
  ...
  fprintf(stderr,"\t-g : Generate one file with the profile of each gene.\n");
			      ...
			      int* L)
{
  ...
  sprintf(USAGE[0],
	  ...
	  PRODUCETSSPLOTS);
  sprintf(USAGE[1],
	  "NAME\n\t%s - a program to produce averaged ChIPseq/ATACseq/RNAseq metaplots around the TSS.\n",
	  PRODUCETSSPLOTS);
  sprintf(USAGE[2],
	  "SYNOPSIS\n\t%s [-bBfF <Rcolor>][-g][-l <bp>][-s <reads>][-u][-w <bp>][-v][-h]\n\t<chrom_info> <refGene.txt> <SAM file> <targetGenes.txt> <name> <length>\n",
	  PRODUCETSSPLOTS);
  sprintf(USAGE[3],
  ...
  while ((c = getopt(argc,argv,"b:B:f:F:gl:s:uw:vh")) != -1)
    switch(c)
      {
      case 'b':
	...
	break;
      case 'v': VRB++;
	break;
      case 'h': printHelp_produceTSSplots();
	exit(1);
	break;
      }
  ...
  strcpy(TrackName,argv[optind+4]);
