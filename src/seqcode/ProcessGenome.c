  ...
  if (SPIKEIN == 0)
    {
      ...
      fprintf(outfile,"track type=bedGraph name=%s description=\"%s %s (factor: %f)\" visibility=full color=%s\n",
  ...
  else
    {
      ...
      /* UCSC BedGraph track header: content */
      fprintf(outfile,"track type=bedGraph name=%s description=\"%s %s (spike-in factor: %f)\" visibility=full color=%s\n",
	      TrackName,VERSION,mess,normfactor,TrackColor);
    }
