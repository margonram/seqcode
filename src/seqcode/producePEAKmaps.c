...
int main (int argc, char *argv[])
{
  ...
  sprintf(OutputDirName,"%s_PEAKmap_%d",TrackName,L);
  ...
  /* Output files */
  sprintf(OutputFileName2,"%s/CenterBedPEAK_%s_%d.bed",OutputDirName,TrackName,L);
  sprintf(mess,"Filename for the Peak file + max points %s",OutputFileName2);
  printRes(mess);

  sprintf(OutputFileName3,"%s/HEATmatrix_%s_%d.txt",OutputDirName,TrackName,L);
  sprintf(mess,"Filename for the HEAT matrix %s",OutputFileName3);
  printRes(mess);
  
  sprintf(OutputFileName4,"%s/HEATmatrix_%s_%d_processed.txt",OutputDirName,TrackName,L);
  sprintf(mess,"Filename for the processed HEAT matrix %s",OutputFileName4);
  printRes(mess);
  
  sprintf(OutputFileName5,"%s/HEATmatrix_%s_%d_peaks.txt",OutputDirName,TrackName,L);
  ...
  if (m->nPeaks>0)
    {
      ...
      sprintf(RscriptFileName,"%s/RscriptPEAKmap_%s_%d.txt",OutputDirName,TrackName,L);
