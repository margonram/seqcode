/*************************************************************************
*                                                                        *
*   Module: produceGENEmaps                                              *
*                                                                        *
*   ProduceGENEmaps main program                                         *
*                                                                        *
...
int main (int argc, char *argv[])
{
  ...
  readargv_produceGENEmaps(argc,argv,ChromInfoFile,
			   RefgeneFile,ReadsFile,
			   GenesFile,TrackName,
			   &L);

  printSeqCodeHeader(PRODUCEGENEMAPS);
  StartingTime(mess,m);
  printMess(mess);
  ...
  /* 3. Create the output files for the GENE maps representations*/
  printMess("3. Creating output files");
  
  sprintf(OutputDirName,"%s_GENEmap_%d",TrackName,L);
  ...
  /* Output files */
  sprintf(OutputFileName2,"%s/HEATmatrix_%s_%d.txt",OutputDirName,TrackName,L);
  sprintf(mess,"Filename for the HEAT matrix %s",OutputFileName2);
  printRes(mess);
  
  sprintf(OutputFileName3,"%s/HEATmatrix_%s_%d_processed.txt",OutputDirName,TrackName,L);
  sprintf(mess,"Filename for the processed HEAT matrix %s",OutputFileName3);
  printRes(mess);
  
  sprintf(OutputFileName4,"%s/HEATmatrix_%s_%d_genes.txt",OutputDirName,TrackName,L);
  sprintf(mess,"Filename for the HEAT gene rank %s",OutputFileName4);
  printRes(mess);
  
  sprintf(PlotFileName2,"%s/PlotHEATmap_%s_%d.pdf",OutputDirName,TrackName,L);
  ...
  m->nGenes = ProcessTargetGenesGENEMaps(GenesFile,Transcripts,
				     ChrNames,ChrSizes,
				     GeneNames,GeneChrs,
				     L,READS,
				     OutputFileName2,
  ...
  /* 7. Produce the final file to produce the GENE map */
  if (m->nGenes>0)
    {
      printMess("7. Running R to produce the plots");

      sprintf(RscriptFileName,"%s/RscriptGENEmap_%s_%d.txt",OutputDirName,TrackName,L);
      sprintf(mess,"Preparing the Rscript file %s",RscriptFileName);
      printMess(mess);
      
      CreateRscriptGENEMaps(OutputFileName3,
			RscriptFileName,
			PlotFileName2,
			TrackName,L,
			m->nReads,
