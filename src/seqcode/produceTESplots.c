...
int main (int argc, char *argv[])
{
  ...
  sprintf(OutputDirName,"%s_TESplot_%d",TrackName,L);
  ...
  /* Output files */
  sprintf(OutputFileName,"%s/TESprofile_%s_%d.txt",OutputDirName,TrackName,L);
  sprintf(mess,"Filename for the TES average profile %s",OutputFileName);
  printRes(mess);

  sprintf(PlotFileName,"%s/PlotTESprofile_%s_%d.pdf",OutputDirName,TrackName,L);
  ...
  if (m->nGenes>0)
    {
      ...
      sprintf(RscriptFileName,"%s/RscriptTESprofile_%s_%d.txt",OutputDirName,TrackName,L);
