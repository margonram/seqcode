/*************************************************************************
*                                                                        *
*   Module: produceGENEplots                                             *
*                                                                        *
*   ProduceGENEplots main program                                        *
*                                                                        *
...
int main (int argc, char *argv[])
{
  ...
  readargv_produceGENEplots(argc,argv,
			    ChromInfoFile,RefgeneFile,
			    ReadsFile,GenesFile,
			    TrackName,&L);

  printSeqCodeHeader(PRODUCEGENEPLOTS);
  StartingTime(mess,m);
  printMess(mess);
  ...
  UPSTREAM = (long*) RequestMemoryRegion(L);
  IDEALGENE = (long*) RequestMemoryRegion(L);
  DOWNSTREAM = (long*) RequestMemoryRegion(L);
  ...
  READS = (unsigned int**) RequestMemoryREADS(ChrSizes,ChrNames);
   ...
   /* 3. Create the output files for the GENE representation*/
  printMess("3. Creating output files");

  sprintf(OutputDirName,"%s_GENEplot_%d",TrackName,L);
  ...
  if (mkdir(OutputDirName,0777))
    {
      printMess("Removing the old output folder");
  ...
  /* Output files */
  sprintf(OutputFileName,"%s/GENEprofile_%s_%d.txt",OutputDirName,TrackName,L);
  sprintf(mess,"Filename for the GENE average profile %s",OutputFileName);
  printRes(mess);

  if (FULL)
    {
      sprintf(OutputFileName2,"%s/FULLprofile_%s_%d.txt",OutputDirName,TrackName,L);
      sprintf(mess,"Filename for the TSS full profile %s",OutputFileName2);
      printRes(mess);
    }
  
  sprintf(PlotFileName,"%s/PlotGENEprofile_%s_%d.pdf",OutputDirName,TrackName,L);
  ...
  m->nGenes = ProcessTargetGenesGENE(GenesFile,Transcripts,
				     ChrNames,ChrSizes,
				     GeneNames,GeneChrs,
				     L,READS,
				     UPSTREAM,
  ...
  /* 7. Produce the final file to produce the GENE plot */
  if (m->nGenes>0)
    {
      OutputIdealGeneProfileFile(UPSTREAM,
				 IDEALGENE,
				 DOWNSTREAM,
				 L,OutputFileName,
				 m->nReads);
      ...
      sprintf(RscriptFileName,"%s/RscriptGENEprofile_%s_%d.txt",OutputDirName,TrackName,L);
      sprintf(mess,"Preparing the Rscript file %s",RscriptFileName);
      printRes(mess);
      
      CreateRscriptGENE(OutputFileName,RscriptFileName,PlotFileName,TrackName,L,m->nReads);
