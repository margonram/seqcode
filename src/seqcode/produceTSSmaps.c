/*************************************************************************
*                                                                        *
*   Module: produceTSSmaps                                               *
*                                                                        *
*   ProduceTSSmaps main program                                          *
*                                                                        *
...
int main (int argc, char *argv[])
{
  ...
  readargv_produceTSSmaps(argc,argv,ChromInfoFile,
			  RefgeneFile,ReadsFile,
			  GenesFile,TrackName,
			  &L);

  printMess(PRODUCETSSMAPS);
  StartingTime(mess,m);
  printMess(mess);
  ...
  sprintf(OutputDirName,"%s_TSSmap_%d",TrackName,L);
  ...
  m->nGenes = ProcessTargetGenesMaps(GenesFile,Transcripts,
				     ChrNames,ChrSizes,
				     GeneNames,GeneChrs,
				     L,READS,
				     OutputFileName2,
  ...
  /* 7. Produce the final file to produce the TSS map */
  if (m->nGenes>0)
    {
      printMess("7. Running R to produce the plots");

      sprintf(RscriptFileName,"%s/RscriptTSSmap_%s_%d.txt",OutputDirName,TrackName,L);
      sprintf(mess,"Preparing the Rscript file %s",RscriptFileName);
      printMess(mess);
      
      CreateRscriptMaps(OutputFileName3,
			RscriptFileName,
			PlotFileName2,
			TrackName,L,
			m->nReads,
